...
class _DataFetcherState extends State<DataFetcher> {
  ...
  Future<void> _fetchAndStore() async {
    ...
    try {
      ...
      if (widget.venueId == solitaireId) {
        await _fetchFromJson(); // ✅ سولتير من الجيسون
      } else {
        await _fetchFromNearby(); // ✅ باقي الفنيوز من اللات واللانق
      }
  ...
  Future<void> _fetchFromJson() async {
    ...
    for (final name in places) {
      ...
      // ✅ نبحث بالقرب من مركز المول مع تحديد الاسم لضمان الدقة
      final uri = Uri.https(
        'maps.googleapis.com',
      ...
      await docRef.set({
        ...
        'category_IDs': ['unassigned'], // ✅ array صحيحة
  ...
  // ✅ باقي الفنيوز (باستخدام lat/lng من كولكشن الفنيوز)
  Future<void> _fetchFromNearby() async {
    ...
    for (final p in results) {
      ...
      await docRef.set({
        ...
        'placeImage': photoUrl,
        'venue_ID': widget.venueId,
        'category_IDs': ['unassigned'], // ✅ موحدة مع سولتير
        'address': details['formatted_address'] ?? '',
        'timestamp': FieldValue.serverTimestamp(),
      });
...
class _DataFetcherState extends State<DataFetcher> {
  ...
  Future<void> _fetchFromJson() async {
    ...
    for (final name in places) {
      ...
      final result = await _textSearch('$name Solitaire Mall Riyadh');
      if (result == null) continue;

      final details = await _placeDetails(result['place_id']);
      final photoRef = details['photos']?[0]?['photo_reference'];
      final photoUrl = photoRef != null ? _photoUrl(photoRef) : null;
