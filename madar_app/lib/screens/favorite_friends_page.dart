    ...
    try {
      ...
      for (final c in contacts) {
        ...
        for (final p in c.phones) {
          ...
          items.add(_FavContactItem(name: name, phone: normalized));
        }
      }
      ...
      final selectedPhone = await Navigator.push<String>(
        context,
        MaterialPageRoute(
          builder: (_) => _FavSelectContactPage(
            contacts: items,
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: _phoneFocus.hasFocus ? 'Enter 9 digits' : 'Phone number',
              hintStyle: TextStyle(
                color: Colors.grey[400],
                fontWeight: FontWeight.w400,
              ),
              prefix: (_phoneFocus.hasFocus || _phoneCtrl.text.isNotEmpty)
                  ? Text(
                      '+966 ',
                      style: TextStyle(
                        color: Colors.grey[400],
                  ...
                  : null,
              suffixIcon: _phoneFocus.hasFocus
                  ? IconButton(
              ...
              focusedBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(12),
                borderSide: BorderSide(
                  color: _errorMsg != null ? AppColors.kError : AppColors.kGreen,
                  width: 2,
                ),
              ),
...
class _FavContactItem {
  const _FavContactItem({required this.name, required this.phone});
  final String name;
  final String phone;
}

class _FavSelectContactPage extends StatefulWidget {
  ...
  @override
  State<_FavSelectContactPage> createState() => _FavSelectContactPageState();
}

class _FavSelectContactPageState extends State<_FavSelectContactPage> {
  final _searchCtrl = TextEditingController();
  ...
  Future<void> _checkContacts() async {
    ...
    _checking = true;
    const batchSize = 10;
    final toCheck = widget.contacts.toList();
      ...
      await Future.wait(batch.map((item) async {
        try {
          final q = await FirebaseFirestore.instance
              .collection('users')
              .where('phone', isEqualTo: '+966${item.phone}')
              .limit(1)
              .get();
          if (mounted) setState(() => _inDbStatus[item.phone] = q.docs.isNotEmpty);
        } catch (_) {
          if (mounted) setState(() => _inDbStatus[item.phone] = false);
        }
      }));
    }
    _checking = false;
  ...
  Map<String, List<_FavContactItem>> get _grouped {
    final map = <String, List<_FavContactItem>>{};
  ...
  Color _avatarColor(String name) {
    const colors = [
      ...
      Color(0xFF009688), Color(0xFFE91E63), Color(0xFFFF5722),
    ];
        ...
        children: [
          ...
          Expanded(
            child: widget.contacts.isEmpty
                ? const Center(child: CircularProgressIndicator(color: AppColors.kGreen))
                : keys.isEmpty
                ? Center(child: Text('No contacts', style: TextStyle(color: Colors.grey[600])))
                : ListView.builder(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    itemCount: keys.fold<int>(0, (acc, k) => acc + 1 + grouped[k]!.length),
                    itemBuilder: (context, index) {
                      ...
                      for (final k in keys) {
                        ...
                        if (rowIndex < list.length) {
                          ...
                          final inDb = _inDbStatus[item.phone];
                          final loading = !_inDbStatus.containsKey(item.phone);
                          final fullPhone = '+966${item.phone}';
                          return ListTile(
                            contentPadding: const EdgeInsets.symmetric(vertical: 4),
                            leading: Container(
                              width: 44, height: 44,
                              decoration: BoxDecoration(
                                color: _avatarColor(item.name),
                                shape: BoxShape.circle,
                              ),
                              child: Center(
                                child: Text(
                            ...
                            title: Text(item.name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.black87)),
                            subtitle: Text(fullPhone, style: TextStyle(fontSize: 13, color: Colors.grey[600])),
                            trailing: loading
                                ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.kGreen))
                                : inDb == false
                                ? OutlinedButton(
                                    onPressed: () => widget.onInvite(fullPhone),
                                ...
                                : null,
                            onTap: inDb == true ? () => Navigator.pop(context, fullPhone) : null,
                          );
                        }
                        total += list.length;
                      }
