...
class ConnectorLink {
  final String id;
  final String
  type; // e.g. "stairs", "elevator", "escalator"
...
class _PathOverviewScreenState
  ...
  String _toFNumber(String? raw) {
    ...
    for (final e in ord.entries) {
      if (up.contains(e.key))
        return e.value;
    }

    if (up.contains('الاول') ||
        up.contains('الأول') ||
        up.contains('اول'))
      return '1';
    if (up.contains('الثاني') ||
        up.contains('ثاني'))
      return '2';
    if (up.contains('الثالث') ||
        up.contains('ثالث'))
      return '3';
    if (up.contains('الرابع') ||
        up.contains('رابع'))
      return '4';
    if (up.contains('الخامس') ||
        up.contains('خامس'))
      return '5';
  ...
  Future<bool>
  _checkArSupportForDestination() async {
  ...
  Future<void>
  _refreshArSupport() async {
    if (_arRefreshPending)
      return; // prevent overlapping runs
    _arRefreshPending = true;
    try {
      final supported =
          await _checkArSupportForDestination();
      if (mounted &&
  ...
  double _calculateTotalDistance() {
    double total = 0.0;
    _pathPointsByFloorGltf.forEach((
      floor,
      points,
    ) {
      for (
  ...
  String _displayNameFromEntrance(
    ...
    if (category == 'bathrooms' ||
        type.startsWith('bathroom_')) {
      if (type == 'bathroom_female')
        return 'Female Bathroom';
      if (type == 'bathroom_male')
        return 'Male Bathroom';
      if (type == 'bathroom_shared')
        return 'Bathroom';
    }

    // Prayer rooms
    if (category == 'prayer_rooms' ||
        type.startsWith('prayer_')) {
      if (type == 'prayer_shared')
        return 'Prayer Room';
      if (type == 'prayer_female')
  ...
  bool
  _isPreferenceAvailableForCurrentRoute(
    String prefValue,
  ) {
    ...
    final startF = _fNumberFromLabel(
      startLabel,
    );
    final destF = _fNumberFromLabel(
      destLabel,
    );
    if (startF == null || destF == null)
      return true;
    ...
    bool matchesPref(ConnectorLink c) {
      final t = _normalizeConnectorType(
        c.type,
      );
      final dirOk =
          _connectorDirectionAllowed(
            t,
            startF,
            destF,
          );
      return dirOk &&
          _connectorMatchesPreference(
            t,
            pref,
          );
    }

    return _connectors.any(
      (c) =>
          linksFloors(c) &&
          matchesPref(c),
    );
  ...
  Future<void> _ensureFloorSelected(
    String floorLabel,
  ) async {
    ...
    final desiredUrl =
        _mapUrlForFloorLabel(label);
    if (desiredUrl.isEmpty) return;

    if (_currentFloor.trim() !=
        desiredUrl) {
      setState(
        () =>
            _currentFloor = desiredUrl,
      );
  ...
  Future<void>
  _loadSuggestedMeetingPoint() async {
    ...
    try {
      ...
      for (final doc in snap.docs) {
        ...
        final entranceRaw =
            matchedCandidate['entrance'];
        if (entranceRaw is! Map)
          continue;
      ...
      debugPrint(
        '✅ Suggested meeting poi: $_suggestedMeetingPoi',
      );
    } catch (e) {
      debugPrint(
        '❌ Failed to load suggested meeting point: $e',
      );
    }
  }
  ...
  Map<String, double>?
  _connectorStartBlender;
  Map<String, double>?
  _connectorDestBlender;
  ...
  _floorLabelByFNumber = {};
  final Map<String, List<ConnectorLink>>
  _connectorsByFloorLabel = {};
  final Map<
    String,
    List<Map<String, dynamic>>
  >
  ...
  List<Map<String, double>>
  get _currentPathPointsGltf =>
      _pathPointsByFloorGltf[_currentFNumber()] ??
      const [];
  ...
  Future<void>
  _syncOverlaysForCurrentFloor() async {
    if (!_jsReady) return;
    ...
    if (_pendingUserPinGltf != null &&
        currF == startF) {
      await _pushUserPinToJsPath(
        _pendingUserPinGltf!,
      );
    } else {
      ...
      if (currF == destF &&
        ...
        if (isPoi) {
          ...
          await _webCtrl?.runJavaScript(
            'window.clearDestPinFromFlutter();',
          );
        } else {
  ...
  bool _connectorMatchesPreference(
    String normType,
    String pref,
  ) {
    ...
    if (p.isEmpty || p == 'any')
      return true;
    if (p == 'escalator')
      return t.startsWith('escalator');
    return t == p;
  }

  Future<void>
  _clearUserPinFromJs() async {
    final c = _webCtrl;
  ...
  Future<void> _pushUserPinToJsPath(
    ...
    final originF = _toFNumber(
      _originFloorLabel,
    );
    if (originF.isNotEmpty &&
        originF != _currentFNumber()) {
      await _clearUserPinFromJs();
      return;
    }
  ...
  Future<void> _pushDestPinToJs(
    Map<String, double> gltf,
  ) async {
    final c = _webCtrl;
    if (c == null || !_jsReady) {
      debugPrint(
        '❌ _pushDestPinToJs: webCtrl or jsReady false',
      );
      return;
    }
    final x = gltf['x'];
    ...
    if (x == null ||
        ...
        z == null) {
      debugPrint(
        '❌ _pushDestPinToJs: invalid coordinates $gltf',
      );
      return;
    }
    try {
      await c.runJavaScript(
        'window.setDestPinFromFlutter($x,$y,$z);',
      );
      await c.runJavaScript(
        'window.testDestPin();',
      );
  ...
  Future<void> _loadEntrances() async {
    try {
      ...
      debugPrint(
        '✅ Entrances loaded: ${map.length} POIs with entrances',
      );
      // After building _entrancesByPoi, fetch descriptions for gates
      final gateEntries =
          <Map<String, dynamic>>[];
      _entrancesByPoi.forEach((
        key,
        entrances,
      ) {
      ...
      print(
        '🚪 Found ${gateEntries.length} gate entries',
      );
            ...
            for (final e
              ...
              final entranceName =
                  e['name']
                      ...
                      .toLowerCase();
              e['description'] =
                  descMap[entranceName] ??
                  'Gate';
            }
          } catch (e) {
            debugPrint(
              '⚠️ Failed to fetch gate descriptions: $e',
            );
            for (final e
                in gateEntries) {
              e['description'] = 'Gate';
      ...
      debugPrint(
        '❌ Failed to load entrances: $e',
      );
    }
  }

  Map<String, dynamic>?
  _findServiceDestinationOption(
    String shopId,
  ) {
    final all =
        _getAllPoisFromEntrances();
  ...
  _loadActiveRequests() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        debugPrint(
          '✅ Active requests loaded: ${active.length}',
        );
      }
    } catch (e) {
      debugPrint(
        'Error loading active requests: $e',
      );
    }
  }
  ...
  _getAllPoisFromEntrances() {
    ...
    if (femaleBathrooms.isNotEmpty) {
      final best =
          _pickClosestEntryToCurrentStart(
            femaleBathrooms,
          );
      result.add({
  ...
  Future<void>
  _resolveDestinationFromEntrances() async {
    if (_destPosBlender != null &&
        !_isBlank(_destFloorLabel))
      return;
    ...
    if (_entrancesByPoi.containsKey(
      normName,
    )) {
      final entrances =
          _entrancesByPoi[normName]!;
      ...
      setState(() {
        ...
        if (_destFloorLabel == null ||
            _destFloorLabel!.isEmpty) {
          _destFloorLabel =
              first['floor'];
        }
      });
  ...
  Future<void>
  _maybeComputeAndPushPath() async {
    if (_booting) return;
    ...
    try {
      ...
      if (effectiveStart == null ||
          effectiveDest == null) {
        debugPrint(
          '⚠️ Missing start or destination coordinates',
        );
        await _syncOverlaysForCurrentFloor();
        return;
      }
      ...
      if (startLabel.trim().isEmpty) {
        debugPrint(
          '⚠️ Start floor unknown — abort routing',
        );
        await _syncOverlaysForCurrentFloor();
        return;
      }
      ...
      if (startF == destF) {
        ...
        if (best == null) {
          debugPrint(
            "⚠️ Could not compute a valid connector path.",
          );
          return;
        }
        ...
        _connectorStartBlender = best
            .endpointsByFNumber[startF];
        _connectorDestBlender = best
            .endpointsByFNumber[destF];
...
const timer = setInterval(function() {
  ...
  void initState() {
    ...
    if (widget.originFloorLabel !=
            null &&
        widget
            .originFloorLabel!
            .isNotEmpty) {
      _originFloorLabel =
          widget.originFloorLabel!;
    }
    if (widget.destinationFloorLabel !=
            null &&
        widget
            .destinationFloorLabel!
            .isNotEmpty) {
    ...
    _requestedFloorToken = widget
        .floorSrc
        .trim();
    _requestedFloorIsUrl =
        _looksLikeMapUrl(
          _requestedFloorToken,
        );
  ...
  Future<void> _loadVenueMaps() async {
    ...
    if (widget.venueMaps != null &&
        widget.venueMaps!.isNotEmpty) {
      final List<Map<String, String>>
      convertedMaps = widget.venueMaps!
          .map(
            (m) =>
                Map<
                  String,
    ...
    try {
      ...
      if (data != null &&
        ...
        if (mounted) {
          ...
          if (_desiredStartFloorLabel
              .isNotEmpty) {
            _ensureFloorSelected(
              _desiredStartFloorLabel,
            );
          } else if (widget.floorSrc
                  .trim()
                  .isEmpty &&
              _pendingFloorLabelToOpen !=
                  null) {
  ...
  _isPreferenceAvailableForCurrentTrip(
    ...
    final startFStr = _toFNumber(
      startLabel,
    );
    final destFStr = _toFNumber(
      destLabel,
    );

    if (startFStr.isEmpty ||
        destFStr.isEmpty)
      return true;
    if (startFStr == destFStr)
      return false;

    final startF = int.tryParse(
      startFStr,
    );
    final destF = int.tryParse(
      destFStr,
    );
    if (startF == null || destF == null)
      return true;
    ...
    for (final c in _connectors) {
      ...
      if (!_connectorMatchesPreference(
        normType,
        p,
      ))
        continue;
      if (!_connectorDirectionAllowed(
        normType,
        startF,
        destF,
      ))
      ...
      final keys = c
          ...
          .map((k) => k.toString())
          .toSet();
      if (keys.contains(startFStr) &&
          keys.contains(destFStr))
        return true;
    }
    return false;
  }
  ...
  Future<void> _changePreference(
    ...
    debugPrint(
      '🎛️ changePreference: $_selectedPreference -> $nextPreference',
    );
    ...
    try {
      ...
      debugPrint(
        '⚠️ Failed to recompute route after preference change: $e',
      );
    }
  }
        ...
        >(
          ...
          builder: (context) =>
              _PoiPickerSheet(
                ...
                selectedPoi:
                    currentStartPoi, // ← pass current selection
              ),
        );
      ...
      final pinResult =
          ...
          >(
            ...
            builder: (context) => SetYourLocationDialog(
              ...
              destinationPoiMaterial: widget
                  .destinationPoiMaterial,
              floorSrc: _currentFloor,
              destinationHitGltf: widget
                  .destinationHitGltf,
              destinationFloorLabel: widget
                  .destinationFloorLabel,
...
class _PoiPickerSheet
  ...
  @override
  __PoiPickerSheetState createState() =>
      __PoiPickerSheetState();
}

class __PoiPickerSheetState
    extends State<_PoiPickerSheet> {
  ...
  bool _isSuggestedPoi(
    ...
    final itemMaterial = norm(
      (item['material'] ?? '')
          .toString(),
    );
    final sugMaterial = norm(
      (suggested['material'] ?? '')
          .toString(),
    );
