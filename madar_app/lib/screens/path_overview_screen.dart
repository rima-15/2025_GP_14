...
class _PathOverviewScreenState extends State<PathOverviewScreen> {
  ...
  List<Map<String, dynamic>> _getAllPoisFromEntrances() {
    ...
    final Set<String> addedNonBathroomKeys = {};

    for (final entrances in _entrancesByPoi.values) {
      for (final entrance in entrances) {
        final category = (entrance['category'] ?? '').toString().toLowerCase();
        final serviceType = (entrance['type'] ?? '').toString().toLowerCase();
        final isBathroom =
        ...
        // ─────────────────────────────────────────────────────────
        // BATHROOMS & PRAYER ROOMS → each entrance becomes a separate POI
        // ─────────────────────────────────────────────────────────
        if (isBathroom || isPrayer) {
          String baseName;
          if (isBathroom) {
            if (serviceType == 'bathroom_female') {
              baseName = 'Female Bathroom';
            } else if (serviceType == 'bathroom_male') {
              baseName = 'Male Bathroom';
            } else {
              baseName = 'Bathroom';
            }
          } else {
            // Prayer room
            if (serviceType == 'prayer_female') {
              baseName = 'Female Prayer Room';
            } else if (serviceType == 'prayer_male') {
              baseName = 'Male Prayer Room';
            } else {
              baseName = 'Prayer Room';
            }
          }
          ...
          final uniqueId =
              '${entrance['material']}_${entrance['id'] ?? entrance['x']}_${entrance['y']}_${entrance['z']}';
          ...
          result.add({
            ...
            'categories': entrance['categories'] ?? [],
            'serviceType': serviceType,
            'gender': entrance['gender'],
        ...
        // ─────────────────────────────────────────────────────────
        // NON‑BATHROOM → group by material (first entrance only)
        // ─────────────────────────────────────────────────────────
        else {
          final material = entrance['material']?.toString() ?? '';
          if (material.isEmpty) continue;
          final normKey = _normPoiKey(material);
          if (!addedNonBathroomKeys.contains(normKey)) {
            addedNonBathroomKeys.add(normKey);
            final displayName = _displayNameFromEntrance(entrance, normKey);
            result.add({
              'name': displayName,
              'type': 'poi',
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) => UnityCameraPage(
            ...
            navigationPreference: _selectedPreference,
          ),
        ),
      );
    } else {
...
class __PoiPickerSheetState extends State<_PoiPickerSheet> {
  ...
  bool _isSelected(Map<String, dynamic> item) {
    ...
    final itemId = item['uniqueId']?.toString();
    final selectedId = selected['uniqueId']?.toString();
    if (itemId != null && selectedId != null && itemId == selectedId)
      return true;
            ...
            SliverList(
              delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: () {
                    // 1. Bathrooms / prayer rooms with a nearby landmark
                    if (poi['nearby'] != null &&
                        poi['nearby'].toString().isNotEmpty) {
                      return Text(
                        'Floor: ${_formatFloorLabel(poi['floor'])} · Next to ${poi['nearby']}',
                        style: TextStyle(fontSize: 14, color: Colors.grey[600]),
                      );
                    }

                    // 2. Gates → use description
                    final category = poi['category']?.toString().toLowerCase();
                    if (category == 'gates') {
                      return Text(
                        poi['description']?.toString() ?? 'Gate',
                        style: TextStyle(fontSize: 14, color: Colors.grey[600]),
                      );
                    }

                    // 3. All other POIs → show floor only
                    return Text(
                      'Floor: ${_formatFloorLabel(poi['floor'])}',
                      style: TextStyle(fontSize: 14, color: Colors.grey[600]),
                    );
                  }(),
