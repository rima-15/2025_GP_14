...
class NotificationPreferences {
  ...
  factory NotificationPreferences.fromMap(
    ...
    var trackingRequests = read(
      'trackingRequests',
    );
    var trackingUpdates = read(
      'trackingUpdates',
    );
    var meetingPointInvitations = read(
      'meetingPointInvitations',
    );
    var meetingPointUpdates = read(
      'meetingPointUpdates',
    );
    var refreshLocationRequests = read(
      'refreshLocationRequests',
    );

    final explicitAllow = readOptional(
      'allowNotifications',
    );
    final legacyAllNotifications =
        readOptional(
          'allNotifications',
        );
    ...
    final allowNotifications =
        explicitAllow ??
        legacyAllNotifications ??
        anyCategoryEnabled;

    if (legacyAllNotifications ==
        true) {
      trackingRequests = true;
      trackingUpdates = true;
      meetingPointInvitations = true;
  ...
  NotificationPreferences copyWith({
    ...
    return NotificationPreferences(
      ...
      meetingPointInvitations:
          meetingPointInvitations ??
          this.meetingPointInvitations,
      meetingPointUpdates:
          meetingPointUpdates ??
          this.meetingPointUpdates,
      refreshLocationRequests:
          refreshLocationRequests ??
          this.refreshLocationRequests,
    );
  }
  ...
  load(String uid) async {
    ...
    final rawPrefs =
        doc.data()?[NotificationPreferences
            .fieldName];
    final prefsMap =
        rawPrefs is Map<String, dynamic>
        ? rawPrefs
        : rawPrefs is Map
